...
public class LQ2748 extends ApplicationFuncs
{	

	@Test(testName="LA_12894",dataProvider="getExcelTestData",description ="Updation of single event by turning Allert all ON to OFF")
	public void test_LQ2748(HashMap<String, String> rowMap){
		try {
			openLoginPage();
			
			doLogin(rowMap.get("TeacherName"),rowMap.get("Password"));
			String subject=getRandomString1(rowMap.get("eventType"));
			eventPost_disable_comment(subject,rowMap.get("location"),rowMap.get("Descrpt"),true);
			Edit_calerderevent_on_comment(subject);
			logout();
