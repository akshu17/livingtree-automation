...
public class LQ4329_ScheduleVideoPost extends ApplicationFuncs
{	

	@Test(testName="LA_12190",dataProvider="getExcelTestData",description ="Verify the visibility of Engagement coefficient for a post (post score) in Conversations screen")
	public void test_LQ4329ScheduleVideoPost(HashMap<String, String> rowMap) {
		try {			
			openLoginPage();
			doLogin(rowMap.get("Teacher"),rowMap.get("Password"));
			deleteAllPosts();
			String subject=getRandomString1(rowMap.get("Subject"));
			postScheduleVideo2(USERDIR+rowMap.get("VideoPath"),subject ,rowMap.get("Message"),rowMap.get("GroupName") );		
